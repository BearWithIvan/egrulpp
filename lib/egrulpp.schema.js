...
const SIGN_ENTITY_TYPE = {
	LE: 'ЕДИНЫЙ ГОСУДАРСТВЕННЫЙ РЕЕСТР ЮРИДИЧЕСКИХ ЛИЦ',
	SE: 'ЕДИНЫЙ ГОСУДАРСТВЕННЫЙ РЕЕСТР ИНДИВИДУАЛЬНЫХ ПРЕДПРИНИМАТЕЛЕЙ'
};

const SIGN_SOLE_ENTITY_TYPE = {
	IP: 'Сведения об индивидуальном предпринимателе',
	KFH: 'Сведения о крестьянском (фермерском) хозяйстве, главой которого является'
};

const SE_PREFIX = {
	IP: { full: 'ИНДИВИДУАЛЬНЫЙ ПРЕДПРИНИМАТЕЛЬ', short: 'ИП' },
	KFH: { full: 'ГЛАВА КРЕСТЬЯНСКОГО (ФЕРМЕРСКОГО) ХОЗЯЙСТВА', short: 'ГЛАВА КФХ' }
};
...
module.exports.SE_PREFIX = SE_PREFIX;